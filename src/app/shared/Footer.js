    ...
    return (
        <footer className='pt-16 bg-dark-gray-600 text-white'>
            <Container>
                            ...
                            <ul className='space-y-4'>
                                {section.links.map((link, i) => (
                                    <li key={i} className='text-sm leading-normal'>
                                        <Link href={link.url} className='inline-block hover:text-primary-400 duration-200 cursor-pointer'>{link.name}</Link>
                                    </li>
                                ))}
                            </ul>
                        </div>
                ...
                <p className='pt-[52px] pb-8 md:text-center text-light-gray-200 text-xs leading-normal'>© 2025 companyname.com, Company Name Platforms Limited. All rights reserved</p>
            </Container>
        </footer>
    )
}
